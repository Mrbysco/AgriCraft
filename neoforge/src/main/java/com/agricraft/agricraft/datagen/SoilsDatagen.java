...
public class SoilsDatagen {
	...
	public static void registerSoils(BootstrapContext<AgriSoil> context) {
		...
		mc(context, "sand", AgriSoil.builder().variants(AgriSoilVariant.builder().tag("c:sand").build(), AgriSoilVariant.builder().tag("minecraft:sand").build()).humidity(ARID).acidity(NEUTRAL).nutrients(LOW).growthModifier(0.8).build());
		...
		agricraft(context, "gravel", AgriSoil.builder().variants(AgriSoilVariant.builder().tag("c:gravel").build(), AgriSoilVariant.builder().block("minecraft:gravel").build()).humidity(DRY).acidity(SLIGHTLY_ALKALINE).nutrients(LOW).growthModifier(0.5).build());
	}
	...
	private static void agricraft(BootstrapContext<AgriSoil> context, String soilId, AgriSoil soil) {
		context.register(ResourceKey.create(AgriApi.AGRISOILS, AgriApi.modLocation(soilId)), soil);
	}
