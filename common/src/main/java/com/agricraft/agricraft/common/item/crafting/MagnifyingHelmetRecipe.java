...
public class MagnifyingHelmetRecipe extends CustomRecipe {
	...
	public boolean matches(CraftingInput container, Level level) {
		...
		for (int i = 0; i < container.size(); i++) {
			...
			if (itemStack.getItem() instanceof ArmorItem armorItem && armorItem.getEquipmentSlot() == EquipmentSlot.HEAD && (itemStack.getOrDefault(ModDataComponents.MAGNIFYING.get(), false))) {
				if (helmet) {
					return false;
				} else {
					helmet = true;
	...
	public ItemStack assemble(CraftingInput container, HolderLookup.Provider registryAccess) {
		...
		if (helmet != null && glass != null) {
			...
			copy.set(ModDataComponents.MAGNIFYING.get(), true);
			return copy;
		}
